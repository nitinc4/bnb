...
class _CategoryDetailScreenState extends State<CategoryDetailScreen> {
  ...
  // List to hold subcategories (starts with the passed data, then gets enriched)
  ...
  void initState() {
    ...
    // SCENARIO 1: Has Subcategories -> Fetch their images (Enrichment)
    if (_subCategories.isNotEmpty) {
      _enrichSubCategories();
    } 
    // SCENARIO 2: No Subcategories -> Fetch Products
    else {
      _productsFuture = _api.fetchProducts(categoryId: widget.category.id);
    }
  }

  // Fetch full details (images) for the subcategories
  Future<void> _enrichSubCategories() async {
    setState(() => _isLoadingSubCats = true);
    
    // We only enrich if they don't have images yet. 
    // (Simple check: check the first one)
    if (_subCategories.isNotEmpty && _subCategories[0].imageUrl == null) {
       try {
         final enriched = await _api.enrichCategories(_subCategories);
         if (mounted) {
           setState(() {
             _subCategories = enriched;
           });
         }
       } catch (e) {
         print("Error enriching subcats: $e");
       }
    }
